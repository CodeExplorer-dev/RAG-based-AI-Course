...
const stats = ref([
  ...
  { label: '课件总数', value: 0, icon: 'FolderOpened', bg: '#fef0f0', color: '#f56c6c' },
])
...
onMounted(async () => {
  try {
    ...
    const d = res.data
    if (d) {
      stats.value[0].value = d.users || 0
      stats.value[1].value = d.courses || 0
      stats.value[2].value = d.courseware || 0
    }
  } catch { /* ignore */ }
})
</script>
...
.manage-card :deep(.el-card__header) { font-size: 19px; font-weight: 600; }
</style>
